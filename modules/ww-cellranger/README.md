...
- `skip_on_chemistry_failure` (Boolean, default=false): If true, samples for which Cell Ranger can't automatically detect the chemistry (usually because the data is low quality or not single-cell) succeed with absent count outputs and `chemistry_status="skipped_non_single_cell"` instead of failing. Other Cell Ranger failures will still cause the workflow to quit. See [Graceful chemistry-detection skip](#graceful-chemistry-detection-skip) for details.

...
The detection is heuristic — it greps Cell Ranger's stderr for substrings like "could not detect", "ambiguous chemistry", `NO_INPUT_ANTIBODY_READS`, `TXRNGR10004`, and "read lengths are incompatible". Any failure that doesn't match these markers (OOM, corrupt reference, malformed FASTQ, etc.) still re-raises so real bugs aren't silently swallowed. The marker list may need tuning across Cell Ranger versions; this module has been validated against Cell Ranger 10.0.0.

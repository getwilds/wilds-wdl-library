...
- `skip_on_chemistry_failure` (Boolean, default=false): If true, samples for which Cell Ranger can't assign a chemistry — typically non-single-cell data, but also single-cell data with too few or too short reads — succeed with absent count outputs and `chemistry_status="skipped_non_single_cell"` instead of failing. Other Cell Ranger failures always re-raise. See [Graceful chemistry-detection skip](#graceful-chemistry-detection-skip) for details.
...
When `cellranger count` cannot assign a chemistry to an input — typically because the data isn't single-cell at all (e.g. bulk RNA-seq mixed into an SRA study) or because the reads are too few/too short for any chemistry's minimum requirements — it exits with an error before producing any of its usual outputs. By default, the task re-raises that failure. Setting `skip_on_chemistry_failure = true` makes the task exit cleanly with `chemistry_status = "skipped_non_single_cell"` and no count outputs, so a scatter over many samples can complete even when a subset turn out not to be usable as single-cell input.

> **Note on the "non_single_cell" label:** the status string is `skipped_non_single_cell` even when the underlying cause is short/insufficient reads rather than fundamentally non-single-cell data. The label reflects the *outcome* (Cell Ranger can't process this as single-cell), not a forensic diagnosis of *why*.

The detection is heuristic — it greps Cell Ranger's stderr for substrings like "could not detect", "ambiguous chemistry", `NO_INPUT_ANTIBODY_READS`, `TXRNGR10002` ("low rate of correct barcodes"), `TXRNGR10004` ("read lengths are incompatible"), and a few related phrases. Any failure that doesn't match these markers (OOM, corrupt reference, malformed FASTQ, etc.) still re-raises so real bugs aren't silently swallowed. When the task does re-raise, it writes `failed` to `chemistry_status.txt` before exiting so Cromwell's output-collection phase doesn't choke on a missing file. The marker list may need tuning across Cell Ranger versions; this module has been validated against Cell Ranger 10.0.0.

...
task run_multi {
  meta {
    author: "Emma Bishop"
    email: "ebishop@fredhutch.org"
    description: "Run Cell Ranger multi for multi-modal single-cell analysis with GEX, ADT, and VDJ libraries"
    outputs: {
        results_tar: "Compressed tarball of Cell Ranger multi output directory",
        web_summary: "Web summary HTML file",
        metrics_summary: "Metrics summary CSV file"
    }
  }
  ...
  parameter_meta {
    ref_gex: "GEX reference transcriptome tarball"
    ref_vdj: "VDJ reference tarball"
    config_csv: "Config CSV file for multi mode specifying library configuration (must include paths to FASTQ files and feature references)"
    gex_fastqs: "Array of GEX FASTQ files"
    adt_fastqs: "Optional: Array of ADT FASTQ files"
    vdj_fastqs: "Optional: Array of VDJ FASTQ files"
    sample_id: "Sample ID for output naming"
    cpu_cores: "Number of CPU cores to use"
    memory_gb: "Memory allocation in GB"
  }
  ...
  command <<<
    ...
    # Extract VDJ reference
    mkdir -p vdj_ref
    tar xf "~{ref_vdj}" -C vdj_ref --strip-components 1
    ...
    ~{if defined(vdj_fastqs) then
      "mkdir -p vdj_fastqs\n    cp ~{sep(' ', select_first([vdj_fastqs, []]))} vdj_fastqs/"
      else ""}
  ...
  output {
    File results_tar = "~{sample_id}_outs.tar.gz"
    File web_summary = "~{sample_id}/outs/web_summary.html"
    File metrics_summary = "~{sample_id}/outs/per_sample_outs/~{sample_id}/metrics_summary.csv"
  }

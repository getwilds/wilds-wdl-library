...
task prepare_fastqs {
  meta {
    author: "Emma Bishop"
    email: "ebishop@fredhutch.org"
    description: "Rename FASTQs to Cell Ranger convention: SampleName_S1_L001_R1_001.fastq.gz"
    outputs: {
        renamed_r1_fastqs: "R1 FASTQ files renamed to Cell Ranger convention",
        renamed_r2_fastqs: "R2 FASTQ files renamed to Cell Ranger convention"
    }
  }
  ...
  command <<<
    ...
    # Create folders for FASTQ files
    mkdir -p renamed_r1_fastqs renamed_r2_fastqs
    ...
    for i in "${!R1_FILES[@]}"; do
      ...
      cp "$R1_FILE" "renamed_r1_fastqs/$TARGET_R1"
      cp "$R2_FILE" "renamed_r2_fastqs/$TARGET_R2"
    done
  >>>

  output {
    Array[File] renamed_r1_fastqs = glob("renamed_r1_fastqs/*.fastq.gz")
    Array[File] renamed_r2_fastqs = glob("renamed_r2_fastqs/*.fastq.gz")
  }

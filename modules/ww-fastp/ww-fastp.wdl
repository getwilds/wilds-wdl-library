...
task fastp_paired {
  ...
  parameter_meta {
    ...
    umi_loc: "fastp --umi_loc value (read1, read2, per_read, index1, index2, per_index). Leave unset to disable UMI extraction. When set, the UMI is moved into the read name with a ':' separator, matching umi_tools dedup."
...
task fastp_single {
  ...
  parameter_meta {
    ...
    length_required: "Minimum read length after trimming (default: 15)"
    adapter_fasta: "Optional FASTA file with custom adapter sequences"
    umi_loc: "fastp --umi_loc value (read1, index1, index2, per_index). Leave unset to disable UMI extraction. When set, the UMI is moved into the read name with a ':' separator, matching umi_tools dedup."
    umi_len: "Length of the UMI in basepairs. Required when umi_loc is set; ignored otherwise (default: 6)"

...
task extract_h5 {
  meta {
    ...
    description: "Extract the filtered_feature_bc_matrix.h5 file from a Cell Ranger count output tarball (e.g. from ww-cellranger run_count) so it can be passed directly to run_seurat"
    ...
    topic: "transcriptomics,single_cell_sequencing"
    species: "human,eukaryote"
    operation: "data_handling"
    input_sample_required: "results_tar:gene_expression_matrix:tar_format"
    input_sample_optional: "none"
    input_reference_required: "none"
    input_reference_optional: "none"
    output_sample: "filtered_h5:gene_expression_matrix:h5"
    output_reference: "none"
  }

  parameter_meta {
    results_tar: "Tarball of cellranger count output folder (contains `outs/` subfolder)"
    sample_id: "Sample ID used to run cellranger count (name of top level folder inside tarball)"
  ...
  command <<<
    set -eo pipefail
    tar -xzf "~{results_tar}" "~{sample_id}/outs/filtered_feature_bc_matrix.h5"

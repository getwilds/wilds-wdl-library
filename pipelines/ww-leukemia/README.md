# ww-leukemia Pipeline
[![Project Status: Experimental – Useable, some support, not open to feedback, unstable API.](https://getwilds.org/badges/badges/experimental.svg)](https://getwilds.org/badges/#experimental)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

A comprehensive WILDS WDL pipeline for genomic analysis using consensus variant calling, structural variant detection, and tumor fraction estimation on targeted panel sequencing data with a focus on leukemia analysis.

## Overview

This pipeline performs end-to-end analysis of human sequencing data with a focus on leukemia analysis. It implements a consensus variant calling approach that combines results from three different variant callers (GATK HaplotypeCaller, GATK Mutect2, and samtools/bcftools) to provide high-confidence detection of single nucleotide variants (SNVs) and small insertions/deletions (indels).

The pipeline is designed for whole genome sequencing and clinical diagnostics, providing comprehensive variant annotation, structural variant detection, tumor fraction estimation, and quality control metrics.
...
This pipeline integrates the following WILDS WDL modules:
...
The pipeline produces comprehensive outputs from each analysis step:
...
The pipeline supports both hg19 and hg38. Ensure all reference files are from the same genome build:
...
The pipeline requires explicit chromosome specification for ichorCNA analysis to ensure compatibility with your sequencing data:
...
The pipeline provides fine-grained control over computational resources through task-category-specific parameters. This allows you to optimize resource usage for different environments (production HPC vs. local testing).
...
**Notes**:
- Resource parameters are **optional** - the pipeline uses production defaults if not specified
...
The pipeline includes a test script with support for execution on multiple WDL backends that runs with minimal test data:
...
- **[WILDS WDL Library](https://github.com/getwilds/wilds-wdl-library)**: Complete collection of WILDS WDL modules and pipelines
- **[WILDS Docker Library](https://github.com/getwilds/wilds-docker-library)**: Container images used by this pipeline

...
def main():
    ...
    # workflow downloads data from ENA rather than using input datasets
...
def setup_inputs(ds: PreprocessDataset):
    ...
    # Parse the comma-separated ENA accession IDs into a list
    ena_accession_list = [id.strip() for id in ds.params["ena_accession_list"].split(",")]
    ds.logger.info(f"Processing {len(ena_accession_list)} ENA samples: {ena_accession_list}")

    inputs = {
        "ena_star.ena_accession_list": ena_accession_list,
        "ena_star.ref_genome": format_inputs_reference_genome(ds),
        "ena_star.sjdb_overhang": ds.params["sjdb_overhang"],
        "ena_star.genome_sa_index_nbases": ds.params["genome_sa_index_nbases"],
        "ena_star.ncpu": ds.params["ncpu"],
        "ena_star.memory_gb": ds.params["memory_gb"],
        "ena_star.ena_file_format": ds.params["ena_file_format"],
        "ena_star.ena_protocol": ds.params["ena_protocol"]
    }
